...
export default function Surah(props) {
	...
	const getAyah = async () => {
		if (props.name === "Para") {
			const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/juz/${props.surah}`)
			const data = await res.json()
			setCurrAyah(data.ayahs[0])
			setS(data.ayahs)
			console.log(data, "line 37")
		} else {
			const res = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/surah/${props.surah}`)
			const data = await res.json()
